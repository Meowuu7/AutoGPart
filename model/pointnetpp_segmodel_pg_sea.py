...
from .utils import farthest_point_sampling, batched_index_select, get_knn_idx, non_maximum_suppression
...
class PointNetPPInstSeg(nn.Module):
    ...
    def forward(
        ...
        scores = torch.max(pred_gt_iou, dim=-1)[0]
        pred_pred_matching_score = torch.matmul(grp_combined.transpose(1, 2), grp_combined)
        pred_pred_iou = torch.div(
            pred_pred_matching_score, torch.clamp(torch.sum(grp_combined, dim=1, keepdim=False).unsqueeze(-1) + torch.sum(grp_combined, dim=1, keepdim=True) - pred_pred_matching_score, min=1e-5)
        )
        for i in range(bz):
            ...
            cur_pred_iou = torch.div(cur_matching_score, torch.clamp(torch.sum(cur_grp_combined, dim=0, keepdim=False).unsqueeze(-1) + torch.sum(masks[i], dim=0, keepdim=True) - cur_matching_score, min=1e-5))
