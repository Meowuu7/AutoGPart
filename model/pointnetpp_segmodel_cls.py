...
class PointNetPPInstSeg(nn.Module):
    ...
    def forward(self, pos: torch.FloatTensor,
        ...
        if inst_seg is None:
            # # bz x N x mask_dim xxx bz x nseeds x mask_dim
            gt_mask_mask_sub_logits = torch.matmul(masks, mask_seed.transpose(1, 2))
        else:
            inst_seg_seed = inst_seg.contiguous().view(bz * N)[fps_idx].contiguous().view(bz, nseeds)
            gt_mask_mask_sub_logits = (inst_seg.contiguous().unsqueeze(2) == inst_seg_seed.contiguous().unsqueeze(1)).float()
            gt_mask_mask_sub_logits = torch.matmul(masks, mask_seed.transpose(1, 2))
        ...
        pred_gt_iou = torch.div(
            pred_grp_matching_score, torch.clamp(torch.sum(grp_combined, dim=1, keepdim=False).unsqueeze(-1) + torch.sum(masks, dim=1, keepdim=True) - pred_grp_matching_score, min=1e-9)
        )

        if torch.any(torch.isnan(pred_gt_iou)):
            print("Nan in pred_gt_iou!")

        pred_gt_iou_np = 1. - pred_gt_iou.detach().cpu().numpy()
