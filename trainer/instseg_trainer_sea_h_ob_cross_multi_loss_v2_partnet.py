...
class TrainerInstSegmentation(nn.Module):
    def __init__(self, dataset_root, num_points=512, batch_size=32, num_epochs=200, cuda=None, dataparallel=False,
        ...
        with FileLock(os.path.expanduser("~/.horovod_lock")):
# Chair;StorageFurniture;Lamp;Bag;Bed;Bottle;Bowl;Clock;Dishwasher;Display;Door;Earphone;Faucet;Hat;Keyboard;Knife;Laptop;Microwave;Mug;Refrigerator;Scissors;Table;TrashCan;Vase
            # train_partnet_shapes = ['StorageFurniture', 'Lamp', 'Chair']
        ...
        self.val_sampler = torch.utils.data.distributed.DistributedSampler(
            self.partnet_val_set, num_replicas=hvd.size(), rank=hvd.rank())
    ...
    def train_all(self):
        ...
        for i_iter in range(eps_training_epochs):
            ...
            # cur_baseline += val_acc - train_acc
            # cur_baseline += val_acc

            # if hvd.rank() == 0:
            #     torch.save(self.model.state_dict(), os.path.join(self.model_dir, "REIN_cur_saved_model.pth"))
        #
        baseline = best_test_acc
