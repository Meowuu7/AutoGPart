...
import torch
from .primitive_dis import ComputePrimitiveDistance
import numpy as np
...
def initialize_open_spline_model(mode=0):
    from model.splinenet import DGCNNControlPoints
    ...
    control_decoder_ = control_decoder.module
...
def construction_affinity_matrix_type(inputs_xyz, type_per_point, T_param_pred, sigma=1.0):
    ...
    distance_matrix = torch.zeros(B, N, N).float().to(T_pred.device)
    masksss_matrix = torch.ones(B, N, N).float().to(T_pred.device)

    for b in range(T_pred.shape[0]):
        type_set = T_pred[b].unique()
        # print(type_set)
        for i in type_set:
            # print(f"Processing primitive with type {i}")
            # point indices of this type
            index = torch.where(T_pred[b] == i)[0]
            primitive_type = int(i)
            #
            inputs_xyz_sub = (inputs_xyz[b])[index]
            inputs_xyz_ = inputs_xyz[b]
            ...
            if primitive_type == 2:  # process open spline
                ...
                # print(param_pred.size(), inputs_xyz_.size())
                distance = routines[primitive_type](points=inputs_xyz_, params=param_pred)
            # print(f"after primitive with type {i} processed")
            # distance_matrix[b, :, index] = distance
            distance_matrix[b, :, index] = distance_matrix[b, :, index] + distance
            masksss_matrix[b, :, index] = 1.0

    # background_mask = distance_matrix == -1

    # type_per_point.size = bz x N
    gt_mask = (type_per_point.unsqueeze(-1) == type_per_point.unsqueeze(1)).float()
    masksss_matrix = masksss_matrix * gt_mask
    selected_distances = torch.sum(torch.sum(distance_matrix * masksss_matrix, dim=-1), dim=-1) / \
        torch.clamp(torch.sum(torch.sum(masksss_matrix, dim=-1), dim=-1), min=1e-9)
    selected_distances = torch.mean(selected_distances)
    ...
    #
    # affinity_matrix = affinity_matrix * masksss_matrix
    #
    # # valid_matrix = (distance_matrix != -1).float()
    # # affinity_matrix = affinity_matrix * valid_matrix
    #
