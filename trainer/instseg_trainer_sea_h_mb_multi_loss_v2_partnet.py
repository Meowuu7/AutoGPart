...
class TrainerInstSegmentation(nn.Module):
    def __init__(self, dataset_root, num_points=512, batch_size=32, num_epochs=200, cuda=None, dataparallel=False,
        ...
        with FileLock(os.path.expanduser("~/.horovod_lock")):
            ...
            if os.path.exists(self.loss_model_save_path_C):
                print(f"=== REMOVE the existing loss model file from {self.loss_model_save_path_C} ===")
                import shutil
                shutil.rmtree(self.loss_model_save_path_C)
        ...
        self.args.loss_model_save_path = self.loss_model_save_path_C
        self.model_C = InstSegNet(
            n_layers=self.n_layers,
            feat_dims=self.feat_dims,
            n_samples=self.n_samples,
            map_feat_dim=self.map_feat_dim,
        ...
        with FileLock(os.path.expanduser("~/.horovod_lock")):
            ...
            train_partnet_shapes_1 = ['StorageFurniture']
            train_partnet_shapes_2 = ['Chair']
            train_partnet_shapes_3 = ['Lamp']

            self.partnet_train_set_1 = PartNetInsSeg(
                root_dir=self.dataset_root, split='train', normalize=True, transform=None, shape=train_partnet_shapes_1,
                level=3, cache_mode=False
            )

            self.partnet_train_set_2 = PartNetInsSeg(
                root_dir=self.dataset_root, split='train', normalize=True, transform=None, shape=train_partnet_shapes_2,
                level=3, cache_mode=False
            )

            self.partnet_train_set_3 = PartNetInsSeg(
                root_dir=self.dataset_root, split='train', normalize=True, transform=None, shape=train_partnet_shapes_3,
                level=3, cache_mode=False
            )
        ...
        self.train_sampler_1 = torch.utils.data.distributed.DistributedSampler(
            self.partnet_train_set_1, num_replicas=hvd.size(), rank=hvd.rank())

        self.train_sampler_2 = torch.utils.data.distributed.DistributedSampler(
            self.partnet_train_set_2, num_replicas=hvd.size(), rank=hvd.rank())

        self.train_sampler_3 = torch.utils.data.distributed.DistributedSampler(
            self.partnet_train_set_3, num_replicas=hvd.size(), rank=hvd.rank())

        self.train_loader_1 = data.DataLoader(
            self.partnet_train_set_1, batch_size=self.batch_size,
            sampler=self.train_sampler_1, **kwargs)

        self.train_loader_2 = data.DataLoader(
            self.partnet_train_set_2, batch_size=self.batch_size,
            sampler=self.train_sampler_2, **kwargs)

        self.train_loader_3 = data.DataLoader(
            self.partnet_train_set_3, batch_size=self.batch_size,
        ...
        #### `In model loss model` is `true` here? ####
    ...
    def _train_one_epoch(
        ...
        #### SET train bars and samplers for dataloders ####
        train_bars = []
        for cur_loader in cur_loaders:
            train_bars.append(tqdm(cur_loader))
        ...
        for train_bar in train_bars:
            for batch_data in train_bar:
                ...
                # batch_momasks = batch_data['gt_mask']
                # batch_valid = batch_data['gt_valid']

                # if batch_momasks.size(1) > batch_moma
                ...
                # GET inst-seg in long type
                # _, batch_inst_seg = torch.max(batch_momasks, dim=-1)
                # MOVE inst-seg to CUDA
                batch_inst_seg = batch_inst_seg.long().cuda()
                # batch_momasks = batch_momasks.long().cuda()
                # batch_valid = batch_valid.long().cuda()
                batch_momasks = get_one_hot(batch_inst_seg, 200)

                # print(batch_pos.size(), batch_inst_seg.size(), batch_momasks.size())
                ...
                rt_values = cur_model(
                    ...
                    loss_model=self.loss_model if not self.in_model_loss_model else self.model.intermediate_loss
                )
                ...
                if "iou" in losses:
                    ...
                    #     continue
                    # print(iou_value)
                    # if iou_value.size(0) > 1:
                    #     iou_value = torch.mean(iou_value)
                ...
                loss = gt_l + neg_iou_loss  # + seg_loss.mean()
                # loss = neg_iou_loss
        ...
        for i_eps in range(tot_eps):
            ...
            for i_model in range(n_models_per_eps):
                ...
                if i_eps >= 0:
                    ...
                    self.model_C.cuda()  # .to(self.device)
                    ''' LOAD model '''
                    ...
                    if not self.in_model_loss_model:
                        ...
                        self.model_C.intermediate_loss.save_init_head_optimizer_by_operation_dicts(
                            cur_selected_loss_dict_list)
                        self.model_C.intermediate_loss.load_head_optimizer_by_operation_dicts(cur_selected_loss_dict_list,
                                                                                            init_lr=self.init_lr,
                                                                                            weight_decay=self.weight_decay)
                    best_test_acc = 0.0
                    ...
                    best_test_acc /= (3 * eps_training_epochs)
                    ...
                    if hvd.rank() == 0:
                        ...
                        self.model_B.intermediate_loss.save_head_optimizer_by_operation_dicts(
                            cur_selected_loss_dict_list, i_model)
                        self.model_C.intermediate_loss.save_head_optimizer_by_operation_dicts(
                            cur_selected_loss_dict_list, i_model)
            ...
            if not self.in_model_loss_model:
                ...
                self.model_C.intermediate_loss.save_init_head_optimizer_by_operation_dicts(baseline_loss_dict, )
                # self.model.cuda()
                self.model_C.intermediate_loss.load_head_optimizer_by_operation_dicts(baseline_loss_dict,
                                                                                    init_lr=self.init_lr,
                                                                                    weight_decay=self.weight_decay)
    ...
    def beam_searh_for_best(self, base_dict=[], max_nn=3, keep_nn=2, conv_select_types=[0, 0, 0]):
        ...
        for j, cur_dict in enumerate(binary_dicts):
            ...
            self.model.intermediate_loss.save_init_head_optimizer_by_operation_dicts(cur_feed_dicts)
            self.model.intermediate_loss.load_head_optimizer_by_operation_dicts(cur_feed_dicts,
                                                                                init_lr=self.init_lr,
                                                                                weight_decay=self.weight_decay)

            self.model_B.intermediate_loss.save_init_head_optimizer_by_operation_dicts(
                cur_feed_dicts)
            self.model_B.intermediate_loss.load_head_optimizer_by_operation_dicts(cur_feed_dicts,
                                                                                  init_lr=self.init_lr,
                                                                                  weight_decay=self.weight_decay)

            self.model_C.intermediate_loss.save_init_head_optimizer_by_operation_dicts(
                cur_feed_dicts)
            self.model_C.intermediate_loss.load_head_optimizer_by_operation_dicts(cur_feed_dicts,
                                                                                  init_lr=self.init_lr,
                                                                                  weight_decay=self.weight_decay)
    ...
    def train_all(self):
        ...
        # baseline_loss_dict = [{"part_oper": self.nn_part_level_operations}]
        ...
        if self.args.resume != "":
            ...
            self.model.load_state_dict(
                torch.load(os.path.join(self.args.resume, "REIN_best_saved_model.pth"))
            )
            self.model.cuda()
        ...
        base_epoch = 0
        base_arch_select_type = baseline_value.tolist()
        for i_iter in range(eps_training_epochs):
            train_acc, train_rigid_loss = self._train_one_epoch(
                base_epoch + i_iter + 1,
                conv_select_types=base_arch_select_type,
