...
class TrainerPrimitiveFitting(nn.Module):
        ...
        with FileLock(os.path.expanduser("~/.horovod_lock")):
            if self.use_ansi:
                ...
                self.train_set = ANSIDataset(
                    ...
                    skip=1, fold=1,
                    prim_types=self.ansi_train_prim_types, split_type=self.split_type, split_train_test=False,
                ...
                self.val_set = ANSIDataset(
                    ...
                    skip=1, fold=1,
                    prim_types=self.ansi_val_prim_types, split_type=self.split_type, split_train_test=False,
    ...
    def rein_train_search_loss(
        ...
        for i_eps in range(tot_eps):
            ...
            for i_iter in range(eps_training_epochs):
                train_acc, train_feat_loss = self._train_one_epoch(
                    base_epoch + i_iter + 1,
                    conv_select_types=base_arch_select_type,
                    loss_selection_dict=baseline_loss_dict
                )

                val_acc, val_feat_loss = self._test(
                    base_epoch + i_iter + 1, desc="val",
                    conv_select_types=base_arch_select_type,
                    loss_selection_dict=baseline_loss_dict
                )
    ...
    def train_all(self):
        ...
        if self.test_performance:
            ...
            ''' SET loss dict '''
            baseline_loss_dict = self.baseline_loss_dict
                ...
                if val_feat_loss < best_feat_loss:
                    ...
                    if not_improved_num_epochs >= 20:
                        ...
                        with open(os.path.join(self.model_dir, "logs.txt"), "a") as wf:
                            ...
                            wf.close()
            if self.test_performance and i_iter == eps_training_epochs - 1:
                if hvd.rank() == 0:
                    torch.save(self.model.state_dict(), os.path.join(self.model_dir, "Loss_last_saved_model.pth"))
                print("Best feat loss =", best_feat_loss, "Current feat loss =", best_feat_loss)
                return
